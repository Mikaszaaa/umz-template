...
X_train.describe()
estimator = KerasRegressor(build_fn=create_baseline, epochs=50, verbose=True)
...
X_dev = pd.DataFrame(p, columns=["mileage", "year","engineCapacity"])
...
l = pd.read_csv(os.path.join("test-A", "in.tsv"), header=None, names=[
                "mileage", "year", "brand", "engingeType", "engineCapacity"], sep='\t')
X_test = pd.DataFrame(l, columns=["mileage", "year","engineCapacity"])
